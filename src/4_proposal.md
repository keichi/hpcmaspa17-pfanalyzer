...
Using these outputs from the simulator, users can locate hot-spots and asses
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\linewidth}{lX}
        ...
        Node Selection    & Selects which computing nodes to assign for a job.
                            (\emph{e.g.} Linear, Random, Topology-aware)     \\ \hline
        Process Placement & Determines on which computing node to place a process.
                            (\emph{e.g.} Block, Cyclic, Application-aware)   \\ \hline
        Routing           & Computes a route between a pair of processes.
                            (\emph{e.g.} D-mod-K, S-mod-K, Random, Dynamic)  \\ \hline
    \end{tabularx}
\end{table}
...
simulator waits until the job has finished. Then, the traffic for each link
